...
import { Trash2, FolderOpen, Grid3x3, List, MoreVertical, Pencil, FileDown } from 'lucide-react'
import { toast } from 'sonner'
import { blueprintStorage } from '@/services/storage'
import { exportAllPlansToPdf, type PlanExportData } from '@/services/pdf-export'
...
export function MyFloorplans({ onLoadFloorplan }: MyFloorplansProps) {
  ...
  const handleDownloadAll = async (plans: BlueprintListItem[]) => {
    if (plans.length === 0 || downloadingAll) return
    ...
    try {
      ...
      for (const plan of plans) {
        const detail = await blueprintStorage.get(plan.id)
        if (!detail) continue

        const layoutData = detail.layoutData as {
          floorplan: SavedFloorplan
          items: SerializedItem[]
        }

        const floorplan = new Floorplan()
        floorplan.loadFloorplan(layoutData.floorplan)
        const size = floorplan.getSize()
        const roomCount = floorplan.getRooms().length
        ...
        exportData.push({
          ...
          thumbnailDataUrl: detail.thumbnailUrl || '',
          roomCount,
          floorSizeText: `${Dimensioning.cmToMeasure(size.x)} x ${Dimensioning.cmToMeasure(size.z)}`,
          totalItemCount: (layoutData.items || []).length,
          items: Array.from(itemCounts.entries()).map(([name, quantity]) => ({ name, quantity }))
        })
      }

      exportAllPlansToPdf(exportData)
      toast.success(t('downloadAllSuccess') || 'PDF downloaded', { id: toastId })
    } catch (error) {
      console.error('Failed to generate combined PDF:', error)
      toast.error(t('downloadAllError') || 'Failed to generate PDF', { id: toastId })
    } finally {
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => handleDownloadAll(filteredFloorplans)}
            disabled={downloadingAll || filteredFloorplans.length === 0}
          >
            <FileDown className="h-4 w-4 mr-1.5" />
            {t('downloadAll') || 'Download All (PDF)'}
          </Button>
